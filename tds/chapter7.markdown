...
The TDS WebAPI deployment service is designed to return status information and detailed logs while a deployment is taking place. The WebAPI service exposes the following endpoints:
...
* **public string[] Messages(bool flush)**: Returns any messages in the message log. If flush is true, the message log is emptied. This is used to get any messages generated by the install process. Messages generated by the install process are also written to the Sitecore logs.
	* *Return Value*: An array of messages in the message list. 
* **public string Remove()**: Removes the service assembly from the /bin folder.
